...
const media_configuration = Object.freeze({
  ...
  store_at: process.env.MEDIA_UPLOAD_DIRECTORY as string,
  allowed_file_types: [
    "text/plain",
    "text/csv",
    "image/bmp",
...
const mw_multipart_formdata = (
  ...
  _file_name_prefix: string = media_configuration.file_name_prefix,
  _max_file_size: number = media_configuration.max_file_size,
  _min_file_size: number = media_configuration.min_file_size,
  _max_file_upload_count: number = media_configuration.max_file_upload_count
) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    multer({
      ...
      storage: multer.diskStorage({
        ...
        filename: (_req, _file, cb) => {
          ...
          return cb(
            null,
            `${_file_name_prefix}_${uuidv4()}_${Date.now()}${ext}`
          );
        }
      }),
      // CONFIGURATION OF MULTER FILE FILTER
      fileFilter: (_req, _file, cb: Function) => {
        if (!_req) {
          return cb(null, true);
        }
        ...
        if (
          parseInt(
            (_req.header("content-length") as string).toString().trim(),
            10
          ) < _min_file_size
        ) {
          return cb(
    ...
    }).array(_file_field_name, _max_file_upload_count)(req, res, (_error) => {
      if (_error) {
        delete req.files;
      ...
      if (req.files && Array.isArray(req.files)) {
        for (const v_file of req.files) {
          ...
          files[files.length] = v_file;
        }
        delete req.files;
      }
      req.app_data.validation_data.uploaded_file_list = files;
